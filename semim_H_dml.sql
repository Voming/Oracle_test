...
CREATE OR REPLACE NONEDITIONABLE PROCEDURE pro_board_insert (
    p_writer_key   board.board_writer%TYPE,  -- 작성자 정보
    p_subject_str  board.subject%TYPE, -- 작성 내용
    p_maxcount     NUMBER  --글 몇개 작성한건지
) IS
    v_b             board%rowtype;
...
-- 글쓰기 함수
    ...
    ELSE
        ...
        --댓글위치 다시 꺼내서 실제로 보드에 글 입력
            INSERT INTO board VALUES (
                v_b.board_id,
                p_subject_str || i,
                '내용----' || i, DEFAULT,
                '127.0.0.1',
                p_writer_key, DEFAULT
            );
